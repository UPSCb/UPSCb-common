...
if [[ ! -f $out_dir/.success || $force == true ]]; then
    sh runSortmerna.sh $out_dir/sortmerna $tmp_dir $source_dir/$fread $source_dir/$rread
...
if [[ ! -f $out_dir/.success || $force == true ]]; then
    sh runTrimmomatic.sh $source_dir/$fread $source_dir/$rread $out_dir/trimmomatic $trimming
    touch $out_dir/.success
fi
...
if [[ ! -f $out_dir/.success || $force == true ]]; then
    #statements
    sh runSTAR.sh $source_dir/$fread $source_dir/$rread $genomeFolder/STAR $gff -m $minintron -o $out_dir/STAR
    touch $out_dir/.success
fi
...
sh runCufflinks.sh $out_dir $genomeFolder/genome.fa $alignment
