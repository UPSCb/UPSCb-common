...
"|Sample|Surviving|S%|Dropped|D%|
|----|----|----|----|----|----|----|----|----|"
grep "Input" $1/*.err | awk -F" " '{gsub(/\(/,"");gsub(/\)/,"");smpl=$1;sub(/^.*\//, "", smpl);sub("_trimmomatic.err:Input","",smpl);print "|"smpl"|"$5"|"$6"|"$8"|"$9"|"}' >> $1/trimmomaticStats.txt
