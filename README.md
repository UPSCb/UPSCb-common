...
4. `bash/jobArrayTemplate.sh` the entry point to submitting SLURM job arrays for enhanced paralelism on HPC (High Perf. Compute).
5. `bash/runTemplate.sh` to prototype script to be run on an HPC (High Perf. Compute) using SLURM (a job manager) (old - use the template.sh instead)
6. `bash/semaphore.sh` an entry point to create a semaphore to allow for higher automation of tasks, primarily between servers
7. `bash/template.sh` a boiler plate to avoid rewriting the same all the time. Will run in dry-mode by default, making testing possible and functional deployment faster

---

## Useful links

The [following link](https://umeauniversity-my.sharepoint.com/:f:/r/personal/edpi0005_ad_umu_se/Documents/summary_server_introduction?csf=1&web=1&e=NQW9Gq) will bring you to a server introduction guide with tips about how to connect to our server, how to run slurm jobs properly and how to use AspSeq.
