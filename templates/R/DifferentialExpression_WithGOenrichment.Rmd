...
### Extracting results
````{r contrasts}
InterestCol <- Condition

# Define your contrasts as a list of vectors, each with two values of InterestCol that you wish to compare
contrasts <- list(
...
# Load function to find out the name of columns not identical in two dataframes (I will use it later)
...
# Set name for coulm grouping all variables in the experiment
# Es: column called "Condition", made of Time_Treatment

# Loop through each contrast and apply your function
for (contrast in contrasts) {
  # Create a dynamic variable name based on the contrast elements
  result_name <- gsub(" ", "", paste(contrast, collapse="vs"))
  
  print(result_name)
  
  # Determine the correct baseline
  
  # Here you should replace CHANGEME with the value of InterestCol
  # Then, you should replace Variable1 and Variable2 with the names of the groups that made up the InterestCol
  # An example would be replacing CHANGEME with "Condition", Variable1 with Time and Variable2 with Treatment
  # You can increase or decrease the number of variables depending on how many variables make up InterestCol
  values_contrast <- samples %>% filter(CHANGEME==contrast[1]) %>%
    select(Variable1, Variable2) %>% 
    distinct()
  
  
  
  values_baseline <- samples %>% filter(CHANGEME==contrast[2]) %>%
    select(Variable1, Variable2) %>% 
    distinct()
  
  # Here we find out for which variable do the two conditions compared in the contrst differ
  # In each contrast, the compared conditions should differ for only 1 variable
  difference <- compare_columns(values_contrast, values_baseline)
  
  contrast_name <- paste0(difference, "_", pull(values_contrast[difference]), "_vs_", pull(values_baseline[difference]))
  
  print(paste0("The name of the contrast is ", contrast_name))
  print(paste0("Baseline values are ", values_baseline$Variable1, " and ", values_baseline$Variable2))

  # Rerun deseq2 with the correct baseline
  
  dds <- readRDS(file=here("data/analysis/salmon/dds_merged-tech-reps.rds"))
  dds$Variable1 <- relevel(dds$Variable1, ref = values_baseline$Variable1)
  dds$Variable2 <- relevel(dds$Variable2, ref = values_baseline$Variable2)
  ...
  up <- tibble(Gene = results_list[[result_name]]$up)
  down <- tibble(Gene = results_list[[result_name]]$dn)
