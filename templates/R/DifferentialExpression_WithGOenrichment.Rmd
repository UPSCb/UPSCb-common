...
* Facultative chunk to convert eggNOG emapper annotations to a format compatible with our scripts

````{r convert_emapper, include=FALSE}
emapper <- read_tsv(here("emapper.annotations.gz"), comment="##")

emapper[["query"]] <- emapper[["#query"]]

emapper <- emapper %>% dplyr::select(query, GOs) %>% 
  mutate(Gene=str_replace(query, "\\..*", "")) %>% 
  ...
  rename(GO = GOs) %>% 
  group_by(Gene) %>%
  summarise(GOs = paste(GO, collapse = "|"), .groups = "drop") %>% 
  filter(!GOs=="-")


write_tsv(emapper, here("gopher_format_annotation.tsv"), col_names=FALSE)
````
