...
filelist <- filelist[match(samples$SampleID, basename(dirname(filelist)))]
````
...
````{r}
dat <- tibble(SampleID=colnames(counts),n_reads=colSums(counts)) 

samples %<>% left_join(dat)

ggplot(samples,aes(SampleID, n_reads, fill=Condition)) + 
  geom_col() + 
  scale_y_continuous(name="reads") +
  geom_text(aes(label = paste0(round(n_reads/1e6, 1), "M"), y =0), 
            angle = 90, hjust = 0) +
  theme_bw() + 
...
````{r}
# change based on your metadata, but make sure that the reference level is the one you want to compare to.
samples <- samples %>%
  mutate_at(vars(Time, Treatment, Genotype, Condition), as.factor) %>%
  mutate(Time = relevel(Time, ref = "T0"),
         Treatment = relevel(Treatment, ref = "Control"),
         Genotype = relevel(Genotype, ref = "Col0"),
         Condition = relevel(Condition, ref = "T0_Col0_Control"))
...
abline(lm(log10(colSums(counts(dds))) ~ colMeans(normalizationFactors(dds))), 
       col = "Red", lty = 3)
...
ggplot(tibble(x=1:length(percent),y=cumsum(percent),p=percent),aes(x=x,y=y)) +
  ...
  geom_label(aes(x = horn$n , y = 100,label = 'Horn', vjust = 0)) +
  geom_label(aes(x = elbow, y = 100,label = 'Elbow', vjust = 1)) +
  theme_bw()
````
