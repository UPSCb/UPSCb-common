...
setMethod(f = "createBins", 
          ...
          definition = function(
            ...
            ans_width <- diff(c(0,as.integer(cumsum(rep.int(chunksize[1],gene.nchunk)))))

            IRanges(end=start(r.ex.glist[[1]])+cumsum(ans_width),width=ans_width+1)

            unlist(mapply(":",start(r.ex.glist[[1]]),end(r.ex.glist[[1]]),SIMPLIFY = TRUE))[ans_end]
